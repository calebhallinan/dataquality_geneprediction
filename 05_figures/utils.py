...
def evaluate_model_validation(data_module, model, combined_adata, output_file="evaluation_output.txt"):
    """
    Evaluates the model on the test set and calculates correlations and MSE, saving outputs to a file.
    ...
    - output_file: Path to save printed output (default: evaluation_output.txt)
    ...
    # NOTE: made this edit 10/2 bc when I changed X for rescue experiment by results variable was not in the correct order
    results = pd.DataFrame(all_predictions, index=all_indices)
    # results = results.groupby(results.index).mean()
    results.index = results.index.str.replace("patch_", "").astype(int)
    # sort by index
    ...
    with open(output_file, "w") as f:
        f.write(f"Spearman correlation: {np.nanmean(corrs_spearman)}\n")
        f.write(f"Pearson correlation: {np.nanmean(corrs_pearson)}\n")
